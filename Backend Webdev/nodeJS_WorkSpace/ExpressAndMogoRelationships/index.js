...
mongoose.connect('mongodb://127.0.0.1:27017/myFarmShopApp3')
    .then(()=>{
...
//here we are going to populate the products data along with farm data
...
app.get('/farms/:id/products/new', async(req,res)=>{
    const {id} = req.params;
    const farmData = await Farm.findById(id);
    res.render('products/newProduct.ejs', {category, id, farmData});
})

//This is how you do a bi-direction linkage between 2 related schema...
app.post('/farms/:id/products', async(req, res)=>{
    const {id} = req.params;
    const farmData = await Farm.findById(id);
    const newProduct = new Product({...req.body});
    newProduct.farm = farmData;
    farmData.products.push(newProduct);
    await newProduct.save();
    await farmData.save();
    res.redirect(`/farms/${id}`);
})

//this will trigger the hooks in the schema...
//Note: In the farm schema we have defined a hooks i.e mongoose middleware to delete all linked products with this farm.
app.delete('/farms/:id', async(req,res)=>{   
    const{id} = req.params;
    const resp = await Farm.findByIdAndDelete(id);
    res.redirect('/farms');
})
...
//detail/Show route... that populating farm Details also...
app.get('/product/id/:id', async (req,res)=>{
    const {id} = req.params; //get the id passes in the url
    const foundprod = await Product.findById(id).populate('farm', 'name');  //this will only populate farmid and name.
